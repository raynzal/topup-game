      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
          ...
          <div>
            ...
            <div className="flex gap-3 mt-4">
              {['M', 'T', 'Y', 'I'].map((letter, i) => (
                <div key={i} className="w-8 h-8 bg-dark-800 rounded-lg flex items-center justify-center text-gray-400 hover:bg-blue-500/10 hover:text-blue-400 transition cursor-pointer text-xs font-bold">
                  {letter}
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {[{ href: '/', label: 'nav.home' }, { href: '/#games', label: 'nav.games' }, { href: '/#promo', label: 'nav.promo' }, { href: '/faq', label: 'FAQ' }, { href: '/auth/register', label: 'nav.register' }, { href: '/auth/login', label: 'nav.login' }, { href: '/admin', label: 'nav.admin' }].map((link, i) => (
                <li key={i}>
                  <Link href={link.href} className="text-gray-500 hover:text-blue-400 transition text-sm">{t(link.label)}</Link>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className="border-t border-dark-800 mt-8 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex gap-4 text-sm">
            <Link href="#" className="text-gray-600 hover:text-blue-400 transition">{t('footer.tos')}</Link>
            <Link href="#" className="text-gray-600 hover:text-blue-400 transition">{t('footer.privacy')}</Link>
          </div>
        </div>
      </div>
    </footer>
